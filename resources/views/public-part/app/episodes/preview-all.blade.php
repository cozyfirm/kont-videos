@extends('public-part.layout.layout')

@section('public-content')
    <section class="home-one" id="trailer-overlay">
        <div class="container-sve-epizode" id="demo-modal">
            <div class="cards-container">
                <div class="cardHomeOne" id="trailer-overlay">
                    <img src="{{ asset('img/hero/Kaftan.jpg') }}" class="card-img">
                    <video loop muted preload src=" {{ asset('img/hero/novi.mp4')}}"></video>
                    <div class="cardContent">
                        <h1>Kaftan moda</h1>
                        <p>Sanja Delić</p>
                        <div class="card-btns">
                            <button type="submit" class="btn-primary modaloppener"><i
                                    class="fi fi-bs-play-circle"></i>Trailer</button>
                            <form action="#" method="get">
                                <button class="btn-primary"><i class="fi fi-bs-play-circle"></i>Video</button>
                            </form>

                        </div>
                    </div>

                </div>
                <div class="cardHomeOne">
                    <img src="{{ asset('img/hero/RoberBaric01.JPG') }}" class="card-img">
                    <video loop muted preload src=" {{ asset('img/hero/novi.mp4')}}"></video>
                    <div class="cardContent">
                        <h1>Asimetrično Ratovanje</h1>
                        <p>Robert Barić</p>
                        <div class="card-btns">
                            <button type="submit" class="btn-primary modaloppener"><i
                                    class="fi fi-bs-play-circle"></i>Trailer</button>
                            <form action="#" method="get">
                                <button class="btn-primary"><i class="fi fi-bs-play-circle"></i>Video</button>
                            </form>

                        </div>
                    </div>

                </div>
                <div class="cardHomeOne">
                    <img src="{{ asset('img/hero/Leibach.jpg') }}" class="card-img">
                    <video loop muted preload src=" {{ asset('img/hero/novi.mp4')}}"></video>
                    <div class="cardContent">
                        <h1>Umjetnost i totalitarizmi</h1>
                        <p>Ivan Novak - Leibach</p>
                        <div class="card-btns">
                            <button type="submit" class="btn-primary modaloppener"><i
                                    class="fi fi-bs-play-circle"></i>Trailer</button>
                            <form action="#" method="get">
                                <button class="btn-primary"><i class="fi fi-bs-play-circle"></i>Video</button>
                            </form>

                        </div>
                    </div>

                </div>
                <div class="cardHomeOne">
                    <img src="{{ asset('img/hero/TamaraSkroza.JPG') }}" class="card-img">
                    <video loop muted preload src=" {{ asset('img/hero/novi.mp4')}}"></video>
                    <div class="cardContent">
                        <h1>Novinarska etika</h1>
                        <p>Tamara Skroza</p>
                        <div class="card-btns">
                            <button type="submit" class="btn-primary modaloppener"><i
                                    class="fi fi-bs-play-circle"></i>Trailer</button>
                            <form action="#" method="get">
                                <button class="btn-primary"><i class="fi fi-bs-play-circle"></i>Video</button>
                            </form>

                        </div>
                    </div>
                </div>
                <div class="cardHomeOne">
                    <img src="{{ asset('img/hero/Una Gunjak.jpeg') }}" class="card-img">
                    <video loop muted preload src=" {{ asset('img/hero/novi.mp4')}}"></video>
                    <div class="cardContent">
                        <h1>Umjetnost filmske režije</h1>
                        <p>Una Gunjak</p>
                        <div class="card-btns">
                            <button type="submit" class="btn-primary modaloppener"><i
                                    class="fi fi-bs-play-circle"></i>Trailer</button>
                            <form action="#" method="get">
                                <button class="btn-primary"><i class="fi fi-bs-play-circle"></i>Video</button>
                            </form>

                        </div>
                    </div>
                </div>
                <div class="cardHomeOne">
                    <img src="{{ asset('img/hero/IvanaKorajlic03.JPG') }}" class="card-img">
                    <video loop muted preload src=" {{ asset('img/hero/novi.mp4')}}"></video>
                    <div class="cardContent">
                        <h1>Zarobljena država</h1>
                        <p>Ivana Korajlić</p>
                        <div class="card-btns">
                            <button type="submit" class="btn-primary modaloppener"><i
                                    class="fi fi-bs-play-circle"></i>Trailer</button>
                            <form action="#" method="get">
                                <button class="btn-primary"><i class="fi fi-bs-play-circle"></i>Video</button>
                            </form>

                        </div>
                    </div>
                </div>
                <div class="cardHomeOne">
                    <img src="{{ asset('img/hero/NevenVidakovic.jpg') }}" class="card-img">
                    <video loop muted preload src=" {{ asset('img/hero/novi.mp4')}}"></video>
                    <div class="cardContent">
                        <h1>Deglobalizirani svijet</h1>
                        <p>Neven Vidaković</p>
                        <div class="card-btns">
                            <button type="submit" class="btn-primary modaloppener"><i
                                    class="fi fi-bs-play-circle"></i>Trailer</button>
                            <form action="#" method="get">
                                <button class="btn-primary"><i class="fi fi-bs-play-circle"></i>Video</button>
                            </form>

                        </div>
                    </div>
                </div>
                <div class="cardHomeOne">
                    <img src="{{ asset('img/hero/EmirMerjemic.jpg') }}" class="card-img">
                    <video loop muted preload src=" {{ asset('img/hero/novi.mp4')}}"></video>
                    <div class="cardContent">
                        <h1>Orkestracija u popuarnoj muzici</h1>
                        <p>Emir Mejemić</p>
                        <div class="card-btns">
                            <button type="submit" class="btn-primary modaloppener"><i
                                    class="fi fi-bs-play-circle"></i>Trailer</button>
                            <form action="#" method="get">
                                <button class="btn-primary"><i class="fi fi-bs-play-circle"></i>Video</button>
                            </form>

                        </div>
                    </div>
                </div>
            </div>
        </div>

        <div class="wraper" id="open-modal">
            <div class="content-container">
                <div class="wraper-body">
                    <video loop muted preload controls poster="{{ asset('img/hero/Kaftan.jpg') }}"
                           src=" {{ asset('img/hero/novi.mp4')}}"></video>
                    <div class="contentoverlay">
                        <div class="title">
                            <p>Ivana Korajlić</p>
                            <H1>Zarobljena država</H1>
                        </div>
                        <div class="modal-close" id="modal_close"><span><i class="fi fi-br-circle-xmark"></i></span></div>
                    </div>
                </div>
                <div class="wraper-footer">
                    <div class="header-info">
                        <span>4 cjeline</span>
                        <span>Trajanje: 52 min</span>
                        <div class="stars">
                            <i class="fi fi-br-star"></i>
                            <i class="fi fi-br-star"></i>
                            <i class="fi fi-br-star"></i>
                            <i class="fi fi-br-star"></i>
                            <i class="fi fi-br-star"></i>
                        </div>
                    </div>
                    <div class="description">
                        <h1>Kaftan Studio</h1>
                        <p>Lorem ipsum, dolor sit amet consectetur adipisicing elit. Sunt, ipsum mollitia. Temporibus
                            suscipit quam ea non aut, ad rerum perspiciatis earum dolores pariatur quisquam magnam
                            repudiandae dolorum dignissimos, minus incidunt.</p>
                    </div>
                    <div class="chapter-list">
                        <div class="chapter-el">
                            <div class="no">
                                <h1>1</h1>
                            </div>
                            <div class="slika">
                                <img src="{{ asset('img/hero/Kaftan.jpg') }}" alt="">
                            </div>
                            <div class="body">
                                <div class="info">
                                    <h2>Chapter 1</h2>
                                    <p>12 min</p>
                                </div>
                                <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Facere quaerat doloremque
                                    repellat iure obcaecati, amet dicta.</p>
                            </div>
                        </div>
                        <div class="chapter-el">
                            <div class="no">
                                <h1>2</h1>
                            </div>
                            <div class="slika">
                                <img src="{{ asset('img/hero/Kaftan.jpg') }}" alt="">
                            </div>
                            <div class="body">
                                <div class="info">
                                    <h2>Chapter 2</h2>
                                    <p>12 min</p>
                                </div>
                                <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Facere quaerat doloremque
                                    repellat iure obcaecati, amet dicta.</p>
                            </div>
                        </div>
                        <div class="chapter-el">
                            <div class="no">
                                <h1>3</h1>
                            </div>
                            <div class="slika">
                                <img src="{{ asset('img/hero/Kaftan.jpg') }}" alt="">
                            </div>
                            <div class="body">
                                <div class="info">
                                    <h2>Chapter 3</h2>
                                    <p>12 min</p>
                                </div>
                                <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Facere quaerat doloremque
                                    repellat iure obcaecati, amet dicta.</p>
                            </div>
                        </div>
                    </div>

                </div>
            </div>
        </div>
    </section>

    @include('public-part.app.shared.accordion')
@endsection
